...
myGenePlot <- function(lc = lcpm, genes = "CAGL0B02475g", sp = sample, annotation = cgToSc) {
  ...
  dat <- lc[genes,sp$Sample]   # extract the subset of genes used for plotting
  rownames(dat) <- paste(genes, annotation[genes, ifelse(is.na(cgname),scname,cgname)], sep = " ")
  ...
  p <- p + geom_point() + geom_smooth(method = "loess", formula = y ~ x, aes(x = as.numeric(Timepoint), y = value, color = Genotype)) + 
    ylab("log2 count per million") + facet_wrap( ~ GeneID ) + theme_cowplot() + theme(axis.text.x = element_text(angle = 45, size = rel(0.75), hjust = 1))
  print(p + panel_border())
}
