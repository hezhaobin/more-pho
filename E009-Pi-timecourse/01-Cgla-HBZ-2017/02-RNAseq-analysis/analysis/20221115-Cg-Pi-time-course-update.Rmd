...
myPlotSet <- function(dt, gs){
  ...
  p <- ggplot(tb, aes(x = time, y = lfc)) +
    ...
    scale_x_continuous(minor_breaks = c(0, 20, 30, 45, 60, 90, 120, 150, 180, 240)) +
    labs(x = "Time (min)", y = "log2FC", title = paste(sum(gs), "genes", sep = " ")) +
    theme_cowplot() + background_grid(major = "none", minor = "x")
}
```
...
```
> 1. Groups 1-3 (top row) follow the same pattern, differing only in when they reach the peak.
> 2. Groups 4, while following the same pattern as 1-3, had a much slower rise, and should probably be considered separately in subsequent analyses
> 3. Groups 5 and 6 genes peak at around 45-60 minutes, then start to return to the baseline.

```{r}
pp <- lapply(select(gs$dn, s_1111:s_0010), function(i) myPlotSet(dat, i))
plot_grid(plotlist = pp)
```
> 1. Group 1, which contains the most genes in the down category, shows a distinct rapid down and recovery pattern.
> 2. Group 2 seems to consist of two subgroups (candidate for a K-means clustering with two clusters, or just based on maximum fold change, e.g., > 10 fold down. The recovery is not as prominent.
> 3. Group 3 and 4 goes down and stays down throughout the process. Clearly different from 1 and 2. Group 4 is milder than group 3.
> 4. Group 5 is similar to Group 1, but less drastic changes
> 5. Group 6 is not all that convincing. Could combine with some other groups in gene set analysis.

The next big questions are:

1. Do these different pattern groups enrich for particular functional groups of genes?
1. Where do the canonical PHO genes reside?
