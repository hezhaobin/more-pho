...
```
We can also calculate the medians of the FSC.H for each sample
```{r}
# get population data
...
```{r}
# calculate medians
medFSC <- fsApply(fs.out, function(x) median(x[,"FSC.H"]), use.exprs = TRUE)
hist(medFSC)
```

So we can see that they are not all uniform. This means without correction for cell size, the median wouldn't be truly comparable across samples. A related issue is background subtraction: we used to simply subtract the median of the negative sample from the medians of all positive samples. But that may not be reasonable.


### Updates 2021-10-14
First, after discussing with Brian Metzger, I figured out that their formula is based on the intuition that FSC is proportional to the max 2d projection (area) of a cell, and thus FSC^(3/2) should be roughly proportional to the volume. By contrast, the fluorescent channels should be the cumulative intensity from the whole cell. Therefore dividing FP intensity by FSC^(3/2) should effectively remove the variation due to cell size difference. He did say that the Wittkopp lab later switched to a different method, to be found in their papers post 2015. I'll take a look

A second point relates to my [previous attempt](1attempt). I realized later that because the GFP values were not background subtracted, and since the difference between a cell with Pho4-mNeon and a cell with no GFP at all is about 1,200 vs 800 when the signal-to-noise is good, when I divide RFP by GFP, the latter mainly consists of autofluorescence and thus would scale with cell size.

Let's check this with our negative strains -- yH156, yH372, yH373, all of which have no mNeon
```{r}
p1 <- ggcyto(gs[grepl("156|372|373", sampleNames(gs))], aes(x = FSC.H, y = BL1.H), subset = "singlet") + geom_hex(bins = 64) + facet_wrap(~name, ncol = 4) + scale_x_log10() + theme_bw()
p2 <- ggcyto(gs[grepl("156|372|373", sampleNames(gs))], aes(x = FSC.H, y = YL2.H), subset = "singlet") + geom_hex(bins = 64) + facet_wrap(~name, ncol = 4) + scale_x_log10() + theme_bw()
grid.arrange(as.ggplot(p1), as.ggplot(p2), nrow = 2)
```

Are the cell size distributions similar across samples?


If I give up the idea of calculating the [Pho4] adjusted [PHO5pr-mCherry] value on a single cell basis, and revert to dealing with population stats (e.g. Median), then 

### Earlier attempt {#1attempt}
My original idea was to follow the Metzger et al 2015 paper's protocol and normalize both fluorescent channels by the cell size, approximated by the FSC signal. I had been a bit unsure about how they came up with the formula, i.e. $\frac{logRFP^2/logFSC^3}{logGFP^2/logFSC^3}$. My best guess is that they explored different transformations of the two variables to look for the most linear correlations between the two measures (since FSC is not a direct measure of cell size), and came up with the cubic and square on each. But looking at the GFP vs RFP plot above, I suddenly realized that in our work what we really cared about is the activity of the chimeric Pho4 as defined by RFP / GFP (reporter protein production per cell normalized by chimeric Pho4 expression level). So that's what I'll be using to calculate the median. First, though, I'll need to subtract the background from both the BL1 and YL2 channels.

```{r background_calc}
# first get the names of the samples that serve as background
# for GFP (BL1), we have three samples that have no mNeon tags
# for RFP (YL2), 
```
...
Export the data
```{r}
ex.file = "data/FCS-files/Chimeric Pho4 test_Experiment_Jia_A3-2.fcs"
ex <- read.FCS(ex.file, transformation = FALSE, emptyValue = FALSE) # the original values are already linearized. 
summary(ex)
```
...
```{r}
require(cowplot)
p0 <- autoplot(ex, "FSC-H", "SSC-H", bins = 128) %>% as.ggplot()
p1 <- ggcyto(ex, aes(x = `FSC-H`, y = `FSC-W`)) + geom_hex(bins = 128) + scale_y_continuous(limits = c(0,300))
p1 + theme_cowplot()
plot_grid(p0, p1)
