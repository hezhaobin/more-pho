...
colnames(raw)[3] <- "dRn"
## remove unused wells
blank <- c(paste(LETTERS[3:8],"1",sep=""))
dat <- subset(raw, !is.na(dRn) & !(Well %in% blank))
## plot raw results
p <- ggplot( data = dat, aes( x = Cycle, y = dRn, color = Well ) ) + geom_line() + geom_point( size = 0.5 )
print(p)
```

```{r determine 50% threshold}
...
p <- ggplot( data = norm.res, aes( x = Cycle, y = n.dRn*100, color = Well ) ) + geom_line() + geom_point( size = 0.5 )
print(p)
...
print(p + geom_point( data = half.point, aes( x = Cycle, y = n.dRn*100, color = Well ), size = 3, shape = 1 ))
```
