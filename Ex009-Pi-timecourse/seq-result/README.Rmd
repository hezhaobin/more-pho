...
1. Use `decideTest( fit, method = "global", lfc = 1, p.value = 0.05 )`, which determines significance based on both p-value (adjusted for multiple testing globally) and log fold-change > 1 (more than 2 fold), the top induced genes identified from 2013 and 2017 data are quite consistent.
1. Overall, 2013 data identifies more DE genes, and often produces a higher log fold-change compared with 2017 data.
    I notice that the del80.PHO4 in 2017 data (S37,S38) have higher variance than other samples. Should look into it a bit more.
