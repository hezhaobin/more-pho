...
**RLE**: log ratio of read count per gene to median read count across sample

    as such, the RLE plot visualizes both the deviation in mean 
...
```{r del80_plot_heatmap, fig.height = 8}
...
1. How many genes are turned on at 20', 30', 45', 60', 120', 180'?
1. What's the overlap between the sets?
1. What is the temporal dynamics?

### Single time-point analyses

_Goal_

- Compare different post-stress time-point samples to the pre-stress sample, identify gene induction independently.

```{r single_time_point_analyses}
# 1. collect data and sample sheet
...
# voom transformation to account for variance-mean dependence
v.dge <- voom(dge, design = dsgn, plot = TRUE)
# v.dge.d80 <- voomWithQualityWeights(dge.d80, design = dsgn.d80, plot = TRUE) # incorporate sample weights

# make contrasts
contr.TC <- makeContrasts(
  d80 = wt.del80 - del4.del80,
  wt.t20 = wt.20m - wt.pre,
  wt.t30 = wt.30m - wt.pre,
  wt.t45 = wt.45m - wt.pre,
  wt.t60 = wt.60m - wt.pre,
  wt.t90 = wt.90m - wt.pre,
  wt.t120 = wt.120m - wt.pre,
  wt.t150 = wt.150m - wt.pre,
  wt.t180 = wt.180m - wt.pre,
  wt.t240 = wt.240m - wt.pre,
  del4.t20 = del4.20m - del4.pre,
  del4.t30 = del4.30m - del4.pre,
  del4.t45 = del4.45m - del4.pre,
  del4.t60 = del4.60m - del4.pre,
  del4.t90 = del4.90m - del4.pre,
  del4.t120 = del4.120m - del4.pre,
  del4.t150 = del4.150m - del4.pre,
  del4.t180 = del4.180m - del4.pre,
  del4.t240 = del4.240m - del4.pre,
  dep4.t20 = wt.20m - del4.20m,
  dep4.t30 = wt.30m - del4.30m,
  dep4.t45 = wt.45m - del4.45m,
  dep4.t90 = wt.90m - del4.90m,
  dep4.t120 = wt.120m - del4.120m,
  dep4.t180 = wt.180m - del4.180m,
  levels  = dsgn
)
...
fit <- contrasts.fit(fit, contrasts = contr.TC)
fit <- eBayes( fit )
sig <- decideTests( fit, method = "global", lfc = 1 ) # only include genes more than 2 fold induced
choose <- rowSums(sig.d80[,1:2]) >= 1 # select genes that are deemed significant in at least one exp.
test <- coef(fit.d80)[choose,]
lm <- lm(test[,"Pho4.13"]~test[,"Pho4.17"])
