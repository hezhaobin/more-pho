...
data.path = "/Volumes/rdss_bhe2/Project/E013-Pho4p-evolution/flow-cytometry/data/20220519-EO-chimera-no-mutation-3/"
...
```{r}
sample <- read.csv("20220519-sample-list.csv") %>% 
...
> Several samples have very low cell counts: 231-555, 241-555, 252-555, 251-555-2
...
lgcl <- logicleTransform("induction")
ex <- transform(ex, lgBL1.H = lgcl(`BL1.H`), lgYL2.H = lgcl(`YL2.H`))
fluo.gate <- gate_flowclust_2d(ex, "lgBL1.H", "lgYL2.H", K = 2, quantile = q, target = c(3,4))
ggcyto(ex, aes(x = lgBL1.H, y = lgYL2.H)) + geom_hex(bins = 64) + geom_gate(fluo.gate) + geom_stats()# + scale_x_logicle() + scale_y_logicle()
```
Implement the 2 cluster gates
```{r}
list.redo <- c("251-555-2", "231-555-2", "231-555-3")
...
Check the results
```{r}
ggcyto(gs[list.redo], aes(x = BL1.H, y = YL2.H), subset = "-outlier") + 
...
write_tsv(final, "20220519-gated-median-out.txt")
```
