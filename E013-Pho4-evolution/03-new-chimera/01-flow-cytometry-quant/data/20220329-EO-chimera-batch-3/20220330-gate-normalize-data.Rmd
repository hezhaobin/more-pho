...
author: Bin He
date: "30 mar 2022 (updated `r format(Sys.time(), '%m/%d/%y')`)"
output:
  html_notebook:
    toc: true
    toc_depth: 4
...
data.path = "/Volumes/rdss_bhe2/Project/E013-Pho4p-evolution/flow-cytometry/data/20220329-EO-chimera-batch-3/FCS/"
...
```{r}
sample <- read.csv("20220329-sample-list.csv") %>% 
...
```{r}
p1 <- ggcyto(gs[["NA-156-1"]], aes(x = FSC.H, y = FSC.W), subset = "root") + geom_hex(bins = 128)
p2 <- ggcyto(gs[["NA-156-2"]], aes(x = FSC.H, y = FSC.W), subset = "-outlier") + geom_hex(bins = 128)
...
```{r}
ggcyto(gs[c(paste0("NA-156-", 1:3), sample(shortNames[1:84], 9))], aes(x = BL1.H, y = YL2.H), subset = "-outlier") + geom_hex(bins = 64) +
...
Notice that a couple of samples showed more than one population and the dominant one appears to be different from that in the biological replicates, i.e., 223-555-3 and 251-555-1. Let's see if we can manually fix this.
```{r}
newGate <- list()
newGate[["223-555-3"]] = gate_flowclust_2d(dat[["223-555-3"]], "BL1.H", "YL2.H", K = 2, quantile = 0.95, target = c(3,4))
newGate[["251-555-1"]] = gate_flowclust_2d(dat[["251-555-1"]], "BL1.H", "YL2.H", K = 2, quantile = 0.95, target = c(3,4))
p1 <- ggcyto(gs[["223-555-3"]], aes(x = BL1.H, y = YL2.H), subset = "-outlier") + geom_hex(bins = 64) +
  geom_gate(newGate[["223-555-3"]]) + geom_stats()
p2 <- ggcyto(gs[["251-555-1"]], aes(x = BL1.H, y = YL2.H), subset = "-outlier") + geom_hex(bins = 64) + 
  geom_gate(newGate[["251-555-1"]]) + geom_stats()
plot_grid(as.ggplot(p1), as.ggplot(p2), ncol = 2)
# update the inductionGate object
gs_pop_set_gate(gs[c("223-555-3","251-555-1")], "induction", newGate)
recompute(gs[c("223-555-3", "251-555-1")], "induction")
```
Check the results
```{r}
ggcyto(gs[grep("223|251", shortNames, value = TRUE)], aes(x = BL1.H, y = YL2.H), subset = "-outlier") + 
  geom_hex(bins = 64) + geom_gate("induction") + geom_stats() + 
  facet_wrap(~name, ncol = 6)# + scale_x_logicle() + scale_y_logicle()
```
...
ggcyto(gs[[grep("223|251", shortNames, value = T)]], aes(x = FSC.H), subset = "induction") + geom_density(fill = "forestgreen") + 
...
write_tsv(final, "20220330-gated-median-out.txt")
```
