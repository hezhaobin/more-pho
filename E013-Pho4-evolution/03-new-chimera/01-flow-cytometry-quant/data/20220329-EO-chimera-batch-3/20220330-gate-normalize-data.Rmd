...
p2 <- ggcyto(gs[["188-555-2"]], aes(x = FSC.H, y = FSC.W), subset = "-outlier") + geom_hex(bins = 128)
...
Notice that two samples, i.e. 223-555-3 and 251-555-1 showed two distinct populations, one of which has normal Pho4-GFP but very little _PHO5pr_-RFP. According to Emily, these strains show consistent bimodality behaviors. In P'tase plate assays they tend to give either very high or no activity results. Here we will us an additional 2 cluster gating strategy to select the expressing population. A note should be added for these samples later.
```{r}
newGate <- list()
newGate[["223-555-3"]] = gate_flowclust_2d(dat[["223-555-3"]], "BL1.H", "YL2.H", K = 2, quantile = 0.9, target = c(3,4))
newGate[["251-555-1"]] = gate_flowclust_2d(dat[["251-555-1"]], "BL1.H", "YL2.H", K = 2, quantile = 0.9, target = c(3,4))
...
Check the results
```{r}
ggcyto(gs[grep("223-555|251-555", shortNames, value = TRUE)], aes(x = BL1.H, y = YL2.H), subset = "-outlier") + 
