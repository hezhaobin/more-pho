...
data.path = "/Volumes/rdss_bhe2/Project/E013-Pho4p-evolution/flow-cytometry/data/20220322-EO-chimera-batch-2/FCS_3.1/"
...
inductionGate <- fsApply(dat, function(fr){
  k1 = ifelse(grepl("234-555",identifier(fr)), 2, k)
  openCyto::gate_flowclust_2d(fr, "BL1.H", "YL2.H", K = k1, quantile = q)
})
...
Notice that 218-373 showed more than one population and there is difference among the biological replicates. Let's see if we can manually fix this.
```{r}
newGate <- list()
newGate[["218-373-1"]] = gate_flowclust_2d(dat[["218-373-1"]], "BL1.H", "YL2.H", K = 2, quantile = q, target = c(2.5,2))
newGate[["218-373-2"]] = gate_flowclust_2d(dat[["218-373-2"]], "BL1.H", "YL2.H", K = 2, quantile = q, target = c(2.5,2))
newGate[["218-373-3"]] = gate_flowclust_2d(dat[["218-373-3"]], "BL1.H", "YL2.H", K = 1, quantile = q, target = c(2.5,2))
p1 <- ggcyto(gs[["218-373-1"]], aes(x = BL1.H, y = YL2.H), subset = "-outlier") + geom_hex(bins = 64) +
  geom_gate(newGate[["218-373-1"]]) + geom_stats()
p2 <- ggcyto(gs[["218-373-2"]], aes(x = BL1.H, y = YL2.H), subset = "-outlier") + geom_hex(bins = 64) + 
  geom_gate(newGate[["218-373-2"]]) + geom_stats()
p3 <- ggcyto(gs[["218-373-3"]], aes(x = BL1.H, y = YL2.H), subset = "-outlier") + geom_hex(bins = 64) + 
  geom_gate(newGate[["218-373-3"]]) + geom_stats()
plot_grid(as.ggplot(p1), as.ggplot(p2), as.ggplot(p3), ncol = 3)
# update the inductionGate object
gs_pop_set_gate(gs[grep("218-373", sampleNames(gs), value = TRUE)], "induction", newGate)
recompute(gs[grep("218-373", sampleNames(gs), value = TRUE)], "induction")
```
## Normalization
...
ggplot(ex[sample(1:96, 48)], aes(x = nFSC, y = nRFP/1e3)) + geom_hex(bins = 64) + scale_y_log10() + stat_smooth(method = "lm") +
...
write_tsv(final, "20220327-gated-median-out.txt")
```
