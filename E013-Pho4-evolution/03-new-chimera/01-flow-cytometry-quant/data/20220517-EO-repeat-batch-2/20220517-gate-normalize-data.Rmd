...
data.path = "/Volumes/rdss_bhe2/Project/E013-Pho4p-evolution/flow-cytometry/data/20220517-EO-chimera-no-mutation-2/"
...
```{r}
sample <- read.csv("20220517-sample-list.csv") %>% 
...
```{r}
p1 <- ggcyto(gs[["211-555-3"]], aes(x = FSC.H, y = FSC.W), subset = "root") + geom_hex(bins = 128)
p2 <- ggcyto(gs[["211-555-3"]], aes(x = FSC.H, y = FSC.W), subset = "-outlier") + geom_hex(bins = 128)
...
Notice that the clustering results for 211-555-1/2/3 showed two connected populations. Here we will use a 2 cluster gating strategy to select the expressing population.
```{r message=FALSE}
#ex <- gs_pop_get_data(gs, "singlet")[[1]]
ex <- fs[["211-555-1"]]
lgcl <- logicleTransform("induction")
ex <- transform(ex, lgBL1.H = lgcl(`BL1.H`), lgYL2.H = lgcl(`YL2.H`))
fluo.gate <- gate_flowclust_2d(ex, "lgBL1.H", "lgYL2.H", K = 2, quantile = q)
ggcyto(ex, aes(x = lgBL1.H, y = lgYL2.H)) + geom_hex(bins = 64) + geom_gate(fluo.gate) + geom_stats()# + scale_x_logicle() + scale_y_logicle()
```
Implement the 2 cluster gates
```{r}
newGate <- list()
newGate[["211-555-1"]] = gate_flowclust_2d(dat[["211-555-1"]], "BL1.H", "YL2.H", K = 2, quantile = 0.9)
newGate[["211-555-2"]] = gate_flowclust_2d(dat[["211-555-2"]], "BL1.H", "YL2.H", K = 2, quantile = 0.9)
newGate[["211-555-3"]] = gate_flowclust_2d(dat[["211-555-3"]], "BL1.H", "YL2.H", K = 2, quantile = 0.9)
p1 <- ggcyto(gs[["211-555-1"]], aes(x = BL1.H, y = YL2.H), subset = "-outlier") + geom_hex(bins = 64) +
  geom_gate(newGate[["211-555-1"]]) + geom_stats()
p2 <- ggcyto(gs[["211-555-2"]], aes(x = BL1.H, y = YL2.H), subset = "-outlier") + geom_hex(bins = 64) +
  geom_gate(newGate[["211-555-2"]]) + geom_stats()
p3 <- ggcyto(gs[["211-555-3"]], aes(x = BL1.H, y = YL2.H), subset = "-outlier") + geom_hex(bins = 64) +
  geom_gate(newGate[["211-555-3"]]) + geom_stats()
plot_grid(as.ggplot(p1), as.ggplot(p2), as.ggplot(p3), ncol = 3)
```

update the inductionGate object
```{r}
gs_pop_set_gate(gs[paste("211-555",c(1:3), sep = "-")], "induction", newGate)
recompute(gs[paste("211-555", c(1:3), sep = "-")], "induction")
```

Check the results
```{r}
ggcyto(gs[grep("211-555", shortNames, value = TRUE)], aes(x = BL1.H, y = YL2.H), subset = "-outlier") + 
  geom_hex(bins = 64) + geom_gate("induction") + geom_stats() + 
  facet_wrap(~name, ncol = 3) + theme_bw()
```
...
write_tsv(final, "20220517-gated-median-out.txt")
```
