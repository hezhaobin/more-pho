...
author: Bin He
date: "29 mar 2022 (updated `r format(Sys.time(), '%m/%d/%y')`)"
output:
  html_notebook:
    toc: true
    toc_depth: 4
...
data.path = "/Volumes/rdss_bhe2/Project/E013-Pho4p-evolution/flow-cytometry/data/20220315-EO-chimera-batch-1/FCS_3.1"
...
```{r}
sample <- read.csv("20220315-sample-list.csv") %>% 
...
ggcyto(fs[test], aes(x = FSC.H, y = SSC.H), subset = "root") +
  geom_hex(bins = 64) + geom_gate(outlier.gate) + geom_stats() + facet_wrap(~name, ncol = 2)# + ggcyto_par_set(limits = "instrument")
```
...
```{r include=FALSE, eval=FALSE, fig.width=4, fig.height=4}
...
ggcyto(gs[sample(1:96, 9)], aes(x = FSC.H), subset = "induction") + geom_density(fill = "forestgreen") + 
...
write_tsv(final, "20220329-gated-median-out.txt")
```
