...
```{r}
data.path = "../../data/FCS/20220104-deep-well-shaken/"
...
names <- sampleNames(fs) %>% 
  gsub("20220104 deep well_Experiment_Manual_", "", .) %>% 
  gsub("20220104 deep well_Experiment_Plate_", "", .) %>% 
  gsub(".fcs","",.)
sampleNames(fs) <- names
```
Metadata
```{r}
sample <- read.csv("20220104-sample-list.csv")
rownames(sample) <- sample$file
metaData <- data.frame(labelDescription = c("file", "sample", "row", "col", "name"))
pData(fs) <- sample
...
```{r}
p1 <- autoplot(fs[sampleNames(fs) %in% c("A2","B5","B9")], "BL1.H") + scale_x_logicle() + facet_wrap(~name, nrow = 3)
p2 <- autoplot(fs[sampleNames(fs) %in% c("A2","B5","B9")], "YL2.H") + scale_x_logicle() + facet_wrap(~name, nrow = 3)
...
ggcyto(fs[sampleNames(fs) %in% c("A1", "A2", "B1", "B5")], aes(x = FSC.H, y = SSC.H), subset = "root") +
...
```{r}
p1 <- ggcyto(gs[["A2"]], aes(x = FSC.H, y = FSC.W), subset = "root") + geom_hex(bins = 128)
p2 <- ggcyto(gs[["A2"]], aes(x = FSC.H, y = FSC.W), subset = "-outlier") + geom_hex(bins = 128)
plot_grid(as.ggplot(p1), as.ggplot(p2), ncol = 2)
```
### Singlet
...
**_Update_** switch to a polygon gate as the clustering is not working well.
...
```{r}
ggcyto(gs[grepl("B",sampleNames(gs))], aes(x = BL1.H, y = YL2.H), subset = "singlet") + geom_hex(bins = 64) +
